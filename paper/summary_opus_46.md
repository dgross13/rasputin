...
- [ ] **Grok intel scan dedup fix** — need to add diff check so scanner only surfaces new/changed items vs last delivered scan. Script: `/home/josh/.openclaw/workspace/tools/grok_social_intel.py`
...
4. **Fix Grok intel scan deduplication** — add diff check to `/home/josh/.openclaw/workspace/tools/grok_social_intel.py` so it only surfaces new/changed items vs last delivered scan.
5. **Get Tarasenko contact info from Josh** — need to know who they are, how to reach them, draft outreach for ВНЖ conversion
6. **Await Josh's confirmation on ELF email draft** — ready to send to info@extraditionlaw.net when Josh says go
7. **Manus competitor Phase 0 — write the bible**: Start SPEC.md, CONTRACTS.md, BUILD_STATE.md, shared schemas, task files. ~2-3 hours. Awaiting Josh's final "go."
...
10. **IVF documents next steps**: Josh fills in passport numbers + Sasha's maiden name → notary → apostille → forward to lawyer
11. **Star Citizen**: Clarify with Josh what he wants done
...
16. **When Dad (Lazar) is near release**: Pull top 5 tai chi studios in Toronto + private in-home instructors
17. Josh provides casino admin panel URL + credentials → verify actual player geo/language breakdown
18. Josh forwards draft letters to lawyer → get on letterhead + notarized/apostilled
19. Sasha books appointment with хирург-герниолог for Josh's suspected umbilical hernia
...
22. Coordinate sperm transport from NGC SPb → Almond Blossoms Dubai
23. Sasha baseline bloods (AMH, Day 2-3 scan) if not already done
24. Deep-dive Rivalry Corp financials if Josh wants to pursue acquisition
